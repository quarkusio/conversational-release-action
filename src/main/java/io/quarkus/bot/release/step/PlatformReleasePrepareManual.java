...
@Singleton
@Unremovable
public class PlatformReleasePrepareManual implements StepHandler {
    ...
    public boolean shouldPause(Context context, Commands commands, GitHub quarkusBotGitHub,
        ...
        comment.append("```\n\n");
        try {
            comment.append("* [Create a pull request](https://github.com/quarkusio/quarkus-platform/compare/"
                    + platformPreparationBranch + "..." + issue.getUser().getLogin() + ":quarkus-"
                    + releaseInformation.getVersion() + "?expand=1) targeting branch `" + platformPreparationBranch + "`"
                    + " (or [generic link if targeted link doesn't work](https://github.com/quarkusio/quarkus-platform/pulls))\n");
        } catch (IOException e) {
            comment.append("* [Create a pull request](https://github.com/quarkusio/quarkus-platform/pulls) targeting branch `"
        ...
        if (releaseInformation.isDot0()) {
            ...
            comment.append(Admonitions.warning(
                    ...
                    + "\n\n");
        } else if (releaseInformation.isLtsMaintenanceReleaseWithRegularReleaseCadence()
